"""
TripMind MCP Routing Server — port 8001
...
mcp = FastMCP("tripmind-routing", host="0.0.0.0", port=_PORT)
...
def _geocode_cached(city: str) -> dict[str, Any]:
    try:
        resp = httpx.get(
            ...
            headers={"User-Agent": "TripMind/1.0 (portfolio project)"},
            timeout=10.0,
        )
...
if __name__ == "__main__":
    log.info("Starting TripMind routing MCP server", port=_PORT, ors_key_set=bool(_ORS_KEY))
