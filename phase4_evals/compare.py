...
def _print_head_to_head(h2h: dict, models: list[str]) -> None:
    ...
    for model_a, model_b in combinations(models, 2):
        ...
        t = res.get("ties", 0)
        total = wa + wb + t or 1
        short_a = model_a.replace("tripmind-", "")
        short_b = model_b.replace("tripmind-", "")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="TripMind eval comparison")
