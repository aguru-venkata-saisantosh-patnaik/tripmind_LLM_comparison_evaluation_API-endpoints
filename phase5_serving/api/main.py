"""
TripMind Inference API — FastAPI server wrapping Ollama.

Endpoints:
  GET  /health          — Ollama ping + registered model availability
  GET  /models          — list registered TripMind models with descriptions
...
app = FastAPI(
    title="TripMind Inference API",
    description=(
        "REST interface for TripMind SLMs — three fine-tuned Llama 3.1 8B variants "
...
async def models() -> ModelsResponse:
    """Return the four registered TripMind models and their training descriptions."""
...
async def optimize(req: OptimizeRequest) -> OptimizeResponse:
    ...
    The persona dict must match the TripMind format:
    ```json
    {
